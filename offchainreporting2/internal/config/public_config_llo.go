...
type PublicConfigLLO struct {
	...
	// protocol instance.
	OracleIdentities []OracleIdentityLLO
...
func (c *PublicConfigLLO) N() int {
	return len(c.OracleIdentities)
}

func (c *PublicConfigLLO) CheckParameterBounds() error {
...
func PublicConfigFromContractConfigLLO(skipResourceExhaustionChecks bool, change ContractConfigLLO) (PublicConfigLLO, error) {
	pubcon, _, err := publicConfigFromContractConfigLLO(skipResourceExhaustionChecks, change)
	return pubcon, err
}

func publicConfigFromContractConfigLLO(skipResourceExhaustionChecks bool, change ContractConfigLLO) (PublicConfigLLO, SharedSecretEncryptions, error) {
	if change.OffchainConfigVersion != OffchainConfigVersion {
		return PublicConfigLLO{}, SharedSecretEncryptions{}, fmt.Errorf("unsuppported OffchainConfigVersion %v, supported OffchainConfigVersion is %v", change.OffchainConfigVersion, OffchainConfigVersion)
	}

	oc, err := deserializeOffchainConfig(change.OffchainConfig)
	if err != nil {
		return PublicConfigLLO{}, SharedSecretEncryptions{}, err
	}

	if err := checkIdentityListsHaveNoDuplicatesLLO(change, oc); err != nil {
		return PublicConfigLLO{}, SharedSecretEncryptions{}, err
	}

	// must check that all lists have the same length, or bad input could crash
	// the following for loop.
	if err := checkIdentityListsHaveTheSameLengthLLO(change, oc); err != nil {
		return PublicConfigLLO{}, SharedSecretEncryptions{}, err
	}

	identities := []OracleIdentityLLO{}
	for i := range change.Signers {
		identities = append(identities, OracleIdentityLLO{
			oc.OffchainPublicKeys[i],
			types.OnchainPublicKey(change.Signers[i][:]),
			oc.PeerIDs[i],
			change.CSAPublicKeys[i],
		})
	}
	...
	if err := checkPublicConfigParametersLLO(cfg); err != nil {
		return PublicConfigLLO{}, SharedSecretEncryptions{}, err
	}
...
func checkIdentityListsHaveNoDuplicatesLLO(change ContractConfigLLO, oc offchainConfig) error {
...

	{
		...
		uniqueCSAPublicKeys := map[string]struct{}{}
		for _, csaPublicKey := range change.CSAPublicKeys {
			if _, ok := uniqueCSAPublicKeys[csaPublicKey]; ok {
				return fmt.Errorf("duplicate transmitter '%v'", csaPublicKey)
			}
			uniqueCSAPublicKeys[csaPublicKey] = struct{}{}
		}
	}
...
func checkIdentityListsHaveTheSameLengthLLO(
	change ContractConfigLLO, oc offchainConfig,
) error {
	...
	for _, identityList := range []struct {
		...
		{len(change.CSAPublicKeys) /*              */, "csa public keys"},
...
func checkPublicConfigParametersLLO(cfg PublicConfigLLO) error {
	/////////////////////////////////////////////////////////////////
...
func checkResourceExhaustionLLO(cfg PublicConfigLLO) error {
