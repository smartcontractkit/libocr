...
func (nbmw *NonblockingMetricsWrapper) flushLoop() {
	...
	if nbmw.metricsImpl == nil {
		nbmw.logger.Error("NonblockingMetricsWrapper: Not flushing metrics, the underlying metricsImpl is nil", nil)
		return
	}
		...
		func(fingerprint string, nonblockingMetric *nonblockingMetric) bool {
			...
			if nonblockingMetric.metricImpl == nil {
				...
				if nonblockingMetricVec.metricVecImpl == nil {
					if nbmw.metricsImpl == nil {
						nbmw.logger.Error("NonblockingMetricsWrapper: Could not create new metric vector, the underlying metricsImpl is nil", nil)
						return true
					}
					var err error
...
func (nbv *nonblockingMetricVec) validLabels(labels map[string]string) error {
	...
	for name, val := range labels {
		...
		if !utf8.ValidString(val) {
			return fmt.Errorf("label %q: value %q is not a valid UTF-8 string", name, val)
		}
	}
