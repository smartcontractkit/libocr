...
func RunManagedOCR3_1Oracle[RI any](
	...
	kvStoreFactory ocr3_1types.KeyValueStoreFactory,
	...
	runWithContractConfig(
		...
		func(ctx context.Context, logger loghelper.LoggerWithContext, contractConfig types.ContractConfig) (err error, retry bool) {
			...
			kvStore, err := kvStoreFactory.NewKeyValueStore(sharedConfig.ConfigDigest)
			if err != nil {
				return fmt.Errorf("ManagedOCR3_1Oracle: failed to instantiate a KeyValueStore"), false
			}
			defer loghelper.CloseLogError(
				kvStore,
				logger,
				"ManagedOCR3_1Oracle: error during kvStore.Close()",
			)
