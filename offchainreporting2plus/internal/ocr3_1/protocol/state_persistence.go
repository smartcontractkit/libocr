...
func (state *statePersistenceState[RI]) tryReplay() bool {
	...
	return state.replayVerifiedBlock(state.ctx, block)
}

func (state *statePersistenceState[RI]) replayVerifiedBlock(ctx context.Context, stb StateTransitionBlock) (success bool) {
	stateTransitionInputs := stb.StateTransitionInputs
	seqNr := stateTransitionInputs.SeqNr
	epoch := stateTransitionInputs.Epoch
	round := stateTransitionInputs.Round
	...
	expectedReportsPrecursorDigest := stb.ReportsPrecursorDigest
	expectedStateTransitionOutputDigest := stb.StateTransitionOutputDigest
	...
	// call the plugin now
	reportsPlusPrecursor, ok := common.CallPlugin[ocr3_1types.ReportsPlusPrecursor](
		ctx,
		logger,
		commontypes.LogFields{},
		"StateTransition",
		0, // StateTransition is a pure function and should finish "instantly"
		func(ctx context.Context) (ocr3_1types.ReportsPlusPrecursor, error) {
			return state.reportingPlugin.StateTransition(ctx, roundCtx, query, aos, kvReadWriteTxn)
		},
	)
	if !ok {
		return
	...
	if err != nil {
		logger.Error("could not get write set from kv transaction", commontypes.LogFields{
			"seqNr": seqNr,
			"err":   err,
		})
		return
	...
	actualReportsPrecursorDigest := MakeReportsPlusPrecursorDigest(ogid, seqNr, reportsPlusPrecursor)
	actualStateTransitionOutputDigest := MakeStateTransitionOutputDigest(ogid, seqNr, writeSet)

	if expectedReportsPrecursorDigest != actualReportsPrecursorDigest {
		logger.Error("ReportsPlusPrecursor digests do not match. "+
			"This is commonly caused by non-determinism in the ReportingPlugin.", commontypes.LogFields{
			"expected": expectedReportsPrecursorDigest,
			"actual":   actualReportsPrecursorDigest,
			"seqNr":    seqNr,
		})
		return
	}

	if expectedStateTransitionOutputDigest != actualStateTransitionOutputDigest {
		logger.Error("StateTransitionOutput digests do not match. "+
			"This is commonly caused by non-determinism in the ReportingPlugin.", commontypes.LogFields{
			"expected": expectedStateTransitionOutputDigest,
			"actual":   actualStateTransitionOutputDigest,
			"seqNr":    seqNr,
		})
		return
	}
