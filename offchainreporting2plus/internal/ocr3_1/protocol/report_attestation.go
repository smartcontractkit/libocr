...
func RunReportAttestation[RI any](
	...
	onchainSignVerifier ocr3types.SignVerifier[RI], //ocr3types.OnchainKeyring[RI]
	...
	newReportAttestationState(ctx, chNetToReportAttestation,
		...
		config, contractTransmitter, logger, netSender, onchainSignVerifier,
...
func (repatt *reportAttestationState[RI]) messageReportSignatures(
	...
	repatt.logger.Debug("received MessageReportSignatures", commontypes.LogFields{
		"sender":   sender,
		"msgSeqNr": msg.SeqNr,
	})
	...
	if repatt.rounds[msg.SeqNr].oracles[sender].sentSignatures {
...
func (repatt *reportAttestationState[RI]) eventComputedReports(ev EventComputedReports[RI]) {
	...
	if len(ev.ReportsPlus) == 0 {
		repatt.logger.Info("ReportingPlugin.Reports returned no reports", commontypes.LogFields{
			"seqNr": ev.SeqNr,
		})
	}
