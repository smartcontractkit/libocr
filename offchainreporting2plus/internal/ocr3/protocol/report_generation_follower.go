...
func (repgen *reportGenerationState[RI]) messageStartEpoch(msg MessageStartEpoch[RI], sender commontypes.OracleID) {
	...
	if msg.StartEpochProof.HighestCertified.IsGenesis() {
		...
		repgen.followerState.firstSeqNrOfEpoch = prepareQc.SeqNr
...
func (repgen *reportGenerationState[RI]) startSubsequentFollowerRound() {
	...
	repgen.tryProcessObserveReqPool()
}
...
func (repgen *reportGenerationState[RI]) messageStartRound(msg MessageStartRound[RI], sender commontypes.OracleID) {
	...
	if putResult := repgen.followerState.observeReqPool.Put(msg.SeqNr, sender, msg); putResult != pool.PutResultOK {
...
func (repgen *reportGenerationState[RI]) tryProcessObserveReqPool() {
	if repgen.followerState.phase != repgenFollowerPhaseReady {
		repgen.logger.Debug("cannot process ObserveReqPool, wrong phase", commontypes.LogFields{
			"phase": repgen.followerState.phase,
		})
		return
	}

	poolEntries := repgen.followerState.observeReqPool.Entries(repgen.followerState.seqNr)

	if poolEntries == nil || poolEntries[repgen.l] == nil {

		repgen.logger.Debug("cannot process ObserveReqPool, it's empty", commontypes.LogFields{
	...
	if repgen.followerState.query != nil {
		repgen.logger.Warn("cannot process ObserveReqPool, query already set", commontypes.LogFields{
...
func (repgen *reportGenerationState[RI]) tryProcessProposePool() {
	...
	if repgen.followerState.query == nil {
		...
		repgen.logger.Debug("query is nil", nil)
		return
	}
	...
	repgen.followerState.phase = repgenFollowerPhaseSentPrepare
	repgen.followerState.seqNr = msg.SeqNr
...
func (repgen *reportGenerationState[RI]) tryProcessPreparePool() {
	...
	if byzQuorumSize <= len(prepareQuorumCertificate) {
		newHighestCertifiedTimestamp := HighestCertifiedTimestamp{
			repgen.followerState.seqNr,
			false,
		}
		if !repgen.followerState.cert.Timestamp().Less(newHighestCertifiedTimestamp) {
...
func (repgen *reportGenerationState[RI]) deliver(commit CertifiedPrepareOrCommitCommit) {
	...
	repgen.followerState.observeReqPool.ReapDelivered(repgen.followerState.deliveredSeqNr)
