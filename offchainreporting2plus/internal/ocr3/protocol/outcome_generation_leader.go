...
func (outgen *outcomeGenerationState[RI]) eventComputedValidateVerifyObservation(ev EventComputedValidateVerifyObservation[RI]) {
...

	{
		...
		outgen.subs.Go(func() {
			outgen.backgroundReachedObservationQuorum(
				ctx,
				logger,
				outctx,
				query,
...
func (outgen *outcomeGenerationState[RI]) backgroundReachedObservationQuorum(
	ctx context.Context,
	logger loghelper.LoggerWithContext,
	outctx ocr3types.OutcomeContext,
	query types.Query,
	...
	quorum, ok := outgen.backgroundObservationQuorum(ctx, logger, outctx, query)
	if !ok {
		return
	}
	...
	select {
	case outgen.chLocalEvent <- EventComputedReachedObservationQuorum[RI]{
		outctx.Epoch,
		outctx.SeqNr,
	}:
	case <-ctx.Done():
...
func (outgen *outcomeGenerationState[RI]) eventComputedReachedObservationQuorum(ev EventComputedReachedObservationQuorum[RI]) {
	if outgen.leaderState.phase != outgenLeaderPhaseSentRoundStart {
		outgen.logger.Debug("discarding EventComputedReachedObservationQuorum, wrong phase", commontypes.LogFields{
			"seqNr": outgen.sharedState.seqNr,
			"phase": outgen.leaderState.phase,
		})
		return
	...
	if ev.Epoch != outgen.sharedState.e || ev.SeqNr != outgen.sharedState.seqNr {
		outgen.logger.Debug("discarding EventComputedReachedObservationQuorum from old round", commontypes.LogFields{
			"seqNr":   outgen.sharedState.seqNr,
			"evEpoch": ev.Epoch,
			"evSeqNr": ev.SeqNr,
		})
