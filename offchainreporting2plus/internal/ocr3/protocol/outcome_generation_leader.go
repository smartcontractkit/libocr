...
func (outgen *outcomeGenerationState[RI]) messageObservation(msg MessageObservation[RI], sender commontypes.OracleID) {
	...
	if err := msg.SignedObservation.Verify(outgen.Timestamp(), outgen.sharedState.seqNr, outgen.leaderState.query, outgen.config.OracleIdentities[sender].OffchainPublicKey); err != nil {
