...
type ReportingPluginFactory[RI any] interface {
	...
	NewReportingPlugin(context.Context, ocr3types.ReportingPluginConfig) (ReportingPlugin[RI], ReportingPluginInfo, error)
}
...
type ReportingPlugin[RI any] interface {
	...
	// blockchain).
	//
	// You may assume that the roundCtx.SeqNr is increasing strictly monotonically
	// across the lifetime of a protocol instance.
	//
	// The KeyValueReader gives read access to the key-value store in the state
	// that it is after RoundContext.SeqNr - 1 is committed.
	Query(ctx context.Context, roundCtx RoundContext, keyValueReader KeyValueReader) (types.Query, error)
	...
	// The KeyValueReader gives read access to the key-value store in the state
	// that it is after RoundContext.SeqNr - 1 is committed.
	Observation(ctx context.Context, roundCtx RoundContext, query types.Query, keyValueReader KeyValueReader) (types.Observation, error)
	...
	// The KeyValueReader gives read access to the key-value store in the state
	// that it is after RoundContext.SeqNr - 1 is committed.
	ValidateObservation(ctx context.Context, roundCtx RoundContext, query types.Query, ao types.AttributedObservation, keyValueReader KeyValueReader) error
	...
	// The KeyValueReader gives read access to the key-value store in the state
	// that it is after RoundContext.SeqNr - 1 is committed.
	ObservationQuorum(ctx context.Context, roundCtx RoundContext, query types.Query, aos []types.AttributedObservation, keyValueReader KeyValueReader) (quorumReached bool, err error)

	// StateTransition modifies the state of the Reporting Plugin, typically based
	// the query and the set of attributed observations of the round.
	...
	// that it is after RoundContext.SeqNr - 1 is committed.
	StateTransition(ctx context.Context, roundCtx RoundContext, query types.Query, aos []types.AttributedObservation, keyValueReadWriter KeyValueReadWriter) (ReportsPlusPrecursor, error)
