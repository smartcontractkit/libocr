...
func (o *ocrEndpointV3) start() error {
	...
	for oid, pid := range o.peerMapping {
		...
		lowPriorityStream, err := o.host.NewStream2(
			pid,
			streamNameFromConfigDigestAndPriority(o.configDigest, ragep2p.StreamPriorityLow),
		...
		defaultPriorityStream, err := o.host.NewStream2(
			pid,
			streamNameFromConfigDigestAndPriority(o.configDigest, ragep2p.StreamPriorityDefault),
...
func (o *ocrEndpointV3) translateInboundMessage(inMsg ragep2p.InboundBinaryMessage, priority ocr2types.BinaryMessageOutboundPriority) ocr2types.InboundBinaryMessage {
	switch msg := inMsg.(type) {
	case ragep2p.InboundBinaryMessagePlain:
		return ocr2types.InboundBinaryMessagePlain{msg.Payload, priority}

	case ragep2p.InboundBinaryMessageRequest:
...
type ocrEndpointV3RequestHandle struct {
	priority          ocr2types.BinaryMessageOutboundPriority
	rageRequestHandle ragep2p.RequestHandle
}
...
func streamNameFromConfigDigestAndPriority(cd ocr2types.ConfigDigest, priority ragep2p.StreamPriority) string {
	switch priority {
	case ragep2p.StreamPriorityLow:
		return fmt.Sprintf("ocr/%s/priority=low", cd)
	case ragep2p.StreamPriorityDefault:
