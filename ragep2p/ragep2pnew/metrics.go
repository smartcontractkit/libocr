...
type peerMetrics struct {
	...
	messagesRateLimitRate       prometheus.Gauge
	messagesRateLimitCapacity   prometheus.Gauge
	bytesRateLimitRate          prometheus.Gauge
	bytesRateLimitCapacity      prometheus.Gauge
...
func newPeerMetrics(registerer prometheus.Registerer, logger commontypes.Logger, self types.PeerID, other types.PeerID) *peerMetrics {
	...
	rawconnRateLimitRate := prometheus.NewGauge(prometheus.GaugeOpts{
		...
		Help:        "[deprecated: use ragep2p_peer_bytes_rate_limit_rate instead] The refill rate in bytes per second for the token bucket rate limiting reads from raw connections with the remote peer",
		ConstLabels: labels,
	})
	...
	rawconnRateLimitCapacity := prometheus.NewGauge(prometheus.GaugeOpts{
		...
		Help:        "[deprecated: use ragep2p_peer_bytes_rate_limit_capacity instead] The capacity in bytes for the token bucket rate limiting reads from raw connections with the remote peer",
		ConstLabels: labels,
	})

	metricshelper.RegisterOrLogError(logger, registerer, rawconnRateLimitCapacity, "ragep2p_peer_rawconn_rate_limit_capacity")

	messagesRateLimitRate := prometheus.NewGauge(prometheus.GaugeOpts{
		Name:        "ragep2p_peer_messages_rate_limit_rate",
		Help:        "The aggregate refill rate in messages per second for the token bucket rate limiting reads from streams with the remote peer",
		ConstLabels: labels,
	})

	metricshelper.RegisterOrLogError(logger, registerer, messagesRateLimitRate, "ragep2p_peer_messages_rate_limit_rate")

	messagesRateLimitCapacity := prometheus.NewGauge(prometheus.GaugeOpts{
		Name:        "ragep2p_peer_messages_rate_limit_capacity",
		Help:        "The aggregate capacity in messages for the token bucket rate limiting reads from streams with the remote peer",
		ConstLabels: labels,
	})

	metricshelper.RegisterOrLogError(logger, registerer, messagesRateLimitCapacity, "ragep2p_peer_messages_rate_limit_capacity")

	bytesRateLimitRate := prometheus.NewGauge(prometheus.GaugeOpts{
		Name:        "ragep2p_peer_bytes_rate_limit_rate",
		Help:        "The aggregate refill rate in bytes per second for the token bucket rate limiting reads from streams with the remote peer",
		ConstLabels: labels,
	})

	metricshelper.RegisterOrLogError(logger, registerer, bytesRateLimitRate, "ragep2p_peer_bytes_rate_limit_rate")

	bytesRateLimitCapacity := prometheus.NewGauge(prometheus.GaugeOpts{
		Name:        "ragep2p_peer_bytes_rate_limit_capacity",
		Help:        "The aggregate capacity in bytes for the token bucket rate limiting reads from streams with the remote peer",
		ConstLabels: labels,
	})

	metricshelper.RegisterOrLogError(logger, registerer, bytesRateLimitCapacity, "ragep2p_peer_bytes_rate_limit_capacity")
	...
	return &peerMetrics{
		...
		messagesRateLimitRate,
		messagesRateLimitCapacity,
		bytesRateLimitRate,
...
func (m *peerMetrics) Close() {
	...
	m.registerer.Unregister(m.messagesRateLimitRate)
	m.registerer.Unregister(m.messagesRateLimitCapacity)
	m.registerer.Unregister(m.bytesRateLimitRate)
	m.registerer.Unregister(m.bytesRateLimitCapacity)
	m.registerer.Unregister(m.messageBytes)
}

func (m *peerMetrics) SetRateLimits(messagesTokenBucketAggregate ratelimitaggregator.TokenBucketAggregate, bytesTokenBucketAggregate ratelimitaggregator.TokenBucketAggregate) {
	m.messagesRateLimitRate.Set(messagesTokenBucketAggregate.Rate)
	m.messagesRateLimitCapacity.Set(messagesTokenBucketAggregate.Capacity)
	m.bytesRateLimitRate.Set(bytesTokenBucketAggregate.Rate)
	m.bytesRateLimitCapacity.Set(bytesTokenBucketAggregate.Capacity)
}
